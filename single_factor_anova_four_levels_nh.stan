//single-factor ANOVA with four levels allowing unequal standard deviations
data {
  int N;                                //sample1 size
  real y[N];                            //y values
...
parameters {
  ...
  real<lower=0> sigma_e[4];        //error standard deviations - one for each level
}
model {
  int j;
  ...
  for (i in 1:N){                   //loop through y values
    j<-level[i];
    y[i] ~ normal(alpha[j],sigma_e[j]);  //y[j] has normal likelihood with mean alpha[j] and std deviation sigma[j]
  }
}
