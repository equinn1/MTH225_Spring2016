...
parameters {
  ...
  real beta;                    //slope parameter for parallel regression lines
...
model {
  ...
  for (i in 1:N){                   //loop through y values
    j<-level[i];
    y[i] ~ normal(alpha[j]+beta*x[i],sigma_e);  //y[j] has normal likelihood with mean alpha[j]+beta*x[i] and std deviation sigma
  }
}
generated quantities {
  real d12;
  
  d12<-alpha[1]-alpha[2];    //sample difference between two intercepts
}
